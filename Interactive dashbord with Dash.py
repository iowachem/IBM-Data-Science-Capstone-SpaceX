...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        select_site =spacex_df[spacex_df['Launch Site']==entered_site]
        fig = px.pie(data_frame=select_site,names='class', 
        title='Total Success by Launch site for site {}'.format(entered_site))
        return fig
#for for other part
...
def get_scatter_plot(range_select):
    fig=px.scatter(spacex_df, x='Payload Mass (kg)',y='class',color='Launch Site',
                   title='Correlation between Payload and Success for all site')
    fig.update_xaxes(range=range_select)
    return fig
